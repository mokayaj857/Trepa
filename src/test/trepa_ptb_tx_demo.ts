...
async function main() {
  ...
  const trepaExt: any = paymentPayload.extensions?.trepa || {};
  const payloadData: any = paymentPayload.payload || {};
  ...
  const txDescriptor: any = assembleGuardedWithdrawTx(paymentPayload, runtimeOpts);
