 ...
 */
export async function buildUnsignedSuiTx(descriptor) {
    const tx = new TransactionBlock();
    // Map args: we expect descriptor.args to be an array where object IDs are strings starting with 0x
    const tbArgs = descriptor.args.map((a) => {
        if (typeof a === "string" && a.startsWith("0x"))
            return tx.object(a);
        if (a instanceof Uint8Array)
            return tx.pure(Array.from(a));
        if (Array.isArray(a) && a.every((x) => typeof x === "number"))
            return tx.pure(a);
        if (typeof a === "string" && /^\d+$/.test(a))
            return tx.pure(BigInt(a), "u64");
        return tx.pure(a);
    });
    const target = `${descriptor.packageObjectId}::${descriptor.module}::${descriptor.function}`;
